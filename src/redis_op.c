...
int rop_set_key(redisContext *conn, char *key,char *value)
{
	...
	{
	        fprintf(stderr, "[-][GMS_REDIS] SET %s %s ERROR\n", key,value);
	...
	{
	   printf("set %s %s error",key,value);
	   retn = -2;
	   goto End;
	}
...
int rop_get_key(redisContext *conn, char *key,char *value)
{
	redisReply *reply = NULL;
        int retn = 0;
	...
	{
		fprintf(stderr, "[-][GMS_REDIS] GET key %s ERROR\n", key);
                LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS] GET key %s ERROR %s\n", key, conn->errstr);
                retn = -1;
                goto END;
	}
...
int rop_range_list(redisContext *conn, char *key, int from_pos, int count, RVALUES values, int *get_num)
{
    ...
    *get_num = max_count;
		...
		//LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]LRANGE %s  !%s\n", key, values[0]);
